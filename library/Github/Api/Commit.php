...
class Github_Api_Commit extends Github_Api
{
    ...
    /**
     * Show a specific commit
     * http://developer.github.com/v3/commits/
     *
     * @param   string  $username         the username
     * @param   string  $repo             the repo
     * @param   string  $sha              the commit sha
     * @return array
     */
    public function getCommitDetails($username, $repo, $sha)
    {
        $response = $this->get('repos/'.urlencode($username).'/'.urlencode($repo).'/commits/'.urlencode($sha));
