...
function imageCrop($image,$left,$upper,$right,$lower) {
	$x=imagesx($image);
	$y=imagesy($image);
	//if ($_debug) print "imageCrop x=$x y=$y left=$left upper=$upper right=$right lower=$lower<br>\n";
	$w=abs($right-$left);
	$h=abs($lower-$upper);
...
#	""" load the image data """
		...
		return $this->getImageFromFile($this->imageName);
	}**/
	
	 function openImage() {
#    """ load the image data """
        if ($this->_debug) print "openImage $this->_v_imageFilename<br>\n";
		$stripped = explode('.',$this->_z_filename);
	$ext = end($stripped); 
	
       // return imagecreatefromjpeg($this->_v_imageFilename);
	//   list($root, $ext) = explode(".",$this->_z_filename);
	//   Zend_Debug::dump($root);
	  // exit;
    //if ( !$root) $root = $this->_z_filename;
	...
	case 'JPG':
      return imagecreatefromjpeg($this->_v_imageFilename);
    case 'png':
      return imagecreatefrompng($this->_v_imageFilename);
    case 'gif':
      return imagecreatefromgif($this->_v_imageFilename);
    }
    }    
...
#	""" save the cropped region """
  ...
  function ZoomifyProcess($filename,$path) {
#    """ the method the client calls to generate zoomify metadata """
      $this->_z_filename = $filename;
      $this->_v_imageFilename = $path.$filename;
	  $this->imageName = $path.$filename;
      $this->createDataContainer($filename);
      $this->getImageMetadata();
      $this->processImage();
      $this->saveXMLOutput();
	 // Zend_debug::dump($this->_v_imageFilename);
	}    
}
