...
class Pas_Pdf_Table {
	 ...
	 * @return array(Pas_Pdf_Page)
	 */
	public function render(Pas_Pdf_Page $page, $posX,$posY,$inContentArea=true){
		...
		if($inContentArea){
			$start_y=$posY + $page->getMargin(Pas_Pdf::TOP);
			$max_y=$page->getHeight()- $page->getMargin(Pas_Pdf::BOTTOM)- $page->getMargin(Pas_Pdf::TOP);
		}
		else{
			$start_y=  $posY;
			$max_y  =  $page->getHeight();
			$posX  -=  $page->getMargin(Pas_Pdf::LEFT);
		}
		...
		foreach($this->_rows as $row) {
			...
			if($test>$max_y || $row->hasPageBreak()){
				...
				$page=$nPage;
				$this->_pages[] = $page;
				$y=$page->getMargin(Pas_Pdf::TOP);

				if($this->_headerRow && $this->_repeatHeader){
					$header=$this->_rows[0];//pre-rendered header row (is first row)
					$header->render($page, $posX, $y);
					$y += $header->getHeight()+$header->getBorderLineWidth(Pas_Pdf::BOTTOM);
				}
			}

			$row->render($page, $posX, $y);
			$y += $row->getHeight()+$row->getBorderLineWidth(Pas_Pdf::BOTTOM);
		}
	 ...
	 * Add Header Row
	 *
	 * @param Pas_Pdf_Table_Row $row
	 */
	public function  setHeader(Pas_Pdf_Table_Row $row){
		if(!$this->_autoWidth)
	 ...
	 * @param Pas_Pdf_Table_Row $row
	 */
	public function addRow(Pas_Pdf_Table_Row $row){
	 ...
	 * @param int $index
	 */
	public function replaceRow(Pas_Pdf_Table_Row $row,$index){
		if(!$this->_autoWidth)
			$row->setWidth($this->_width);
	 ...
	 *
	 * @return array(Pas_Pdf_Table_Rows)
	 */
	 ...
	 */
	private function _preRender(Pas_Pdf_Page $page, $posX,$posY,$inContentArea=true){
		...
		foreach($this->_rows as $row) {
			...
			$posY += $row->getHeight()+$row->getBorderLineWidth(Pas_Pdf::BOTTOM);
		}
