...
class Pas_Pdf_Table_Cell {
	 ...
	 * Cell borders
	 *
	 * @var array (Pas_Pdf position=>array('color','width','dashing_pattern'))
	 */
	private $_border;
	 ...
	 * Set Cell Borders
	 *
	 * @param array(array(Pas_Pdf position, Zend_Pdf_Styles style))
	 */
	 ...
	 *
	 * @param string $text
	 * @param Pas_Pdf $align Horizontal Alignment
	 * @param Pas_Pdf $valign Vertical Alignment
	 */
	...
	public function setImage($filename,$align=null,$valign=null,$scale=1){
		...
		if($scale>1)
			throw new Zend_Exception("Scale must be between (0,1]","Pas_Pdf_Table_Cell::addImage()");
	 ...
	 */
	public function preRender(Pas_Pdf_Page $page,$posX,$posY,$inContentArea=true) {
		if(!$this->_width){
			//no width given, get max width of page
			if($inContentArea){
				$width=$page->getWidth()-$posX-($page->getMargin(Pas_Pdf::LEFT)+$page->getMargin(Pas_Pdf::RIGHT));
			}
			else{
		...
		//calc max cell width
		$maxWidth=$width-($this->_padding[Pas_Pdf::LEFT]+$this->_padding[Pas_Pdf::RIGHT])-(+$this->_getBorderLineWidth(Pas_Pdf::LEFT)+$this->_getBorderLineWidth(Pas_Pdf::RIGHT));
		...
		if($this->_text){
			...
			if(!$this->_width){
				//add padding
				$this->_recommendedWidth = $text_props['text_width']+($this->_padding[Pas_Pdf::LEFT]+$this->_padding[Pas_Pdf::RIGHT])+$this->_getBorderLineWidth(Pas_Pdf::LEFT)+$this->_getBorderLineWidth(Pas_Pdf::RIGHT);
			}
			else{
			...
			if(!$this->_height){
				...
				$this->_recommendedHeight = $height+($this->_padding[Pas_Pdf::TOP]+$this->_padding[Pas_Pdf::BOTTOM]);
			}
		...
		elseif($this->_image){
			...
			if(!$this->_width)
				$this->_recommendedWidth = $this->_image['scale']*$image->getPixelWidth ()+($this->_padding[Pas_Pdf::LEFT]+$this->_padding[Pas_Pdf::RIGHT])+$this->_getBorderLineWidth(Pas_Pdf::LEFT)+$this->_getBorderLineWidth(Pas_Pdf::RIGHT);
			if(!$this->_height)
				$this->_recommendedHeight = $this->_image['scale']*$image->getPixelHeight ()+($this->_padding[Pas_Pdf::TOP]+$this->_padding[Pas_Pdf::BOTTOM]);
	 ...
	 * @param int $posY
	 */
	public function render(Pas_Pdf_Page $page, $posX, $posY) {
	...
	private function _renderText(Pas_Pdf_Page $page, $posX, $posY){
		...
		if(count($this->_text['lines'])>1){
			...
			*/
			...
			$this->_vAlign=Pas_Pdf::TOP; //@@TODO ONLY TOP ALIGNEMENT IS VALID AT THIS MOMENT
			foreach ($this->_text['lines'] as $line){
				$page->drawText($line,$this->_getTextPosX($posX), $this->_getTextPosY($page,$y_inc));
				$y_inc+=$line_height;
			}
	...
	private function _renderImage(Pas_Pdf_Page $page, $posX, $posY){
	...
	private function _renderBorder(Pas_Pdf_Page $page, $posX, $posY) {
		...
		foreach ($this->_border as $key=>$style){
			...
			switch ($key) {
				case Pas_Pdf::TOP:
					$page->drawLine(
									$posX, $posY-$this->_getBorderLineWidth(Pas_Pdf::TOP)/2,
									$posX+$this->_width, $posY-$this->_getBorderLineWidth(Pas_Pdf::TOP)/2,
									true
									);
					break;
				case Pas_Pdf::BOTTOM:
					$page->drawLine(
									$posX, $posY + $this->_height+$this->_getBorderLineWidth(Pas_Pdf::BOTTOM)/2,
									$posX+$this->_width, $posY + $this->_height+$this->_getBorderLineWidth(Pas_Pdf::BOTTOM)/2,
									true
									);
					break;
				case Pas_Pdf::RIGHT:
	...
	private function  _renderBackground(Pas_Pdf_Page $page, $posX, $posY){
	...
	private function _getTextPosX($posX){
		...
		switch ($this->_align){
			case Pas_Pdf::RIGHT:
				$x=$posX+$this->_width-$this->_text['width']-$this->_padding[Pas_Pdf::RIGHT]-$this->_getBorderLineWidth(Pas_Pdf::RIGHT)/2;
				break;
			case Pas_Pdf::CENTER:
				$x=$posX+$this->_width/2-$this->_text['width']/2;
				break;
			default: //LEFT
				$x=$posX+$this->_padding[Pas_Pdf::LEFT]+$this->_getBorderLineWidth(Pas_Pdf::LEFT)/2;
				break;
		}
		return $x;
	}
	 ...
	 * @return int
	 */
	private function _getTextPosY(Pas_Pdf_Page $page,$posY){
		$y=0;
		$line_height=$page->getFontHeight()+$this->_textLineSpacing;

		switch ($this->_vAlign){
			case Pas_Pdf::BOTTOM:
				$y=$posY+$this->_height-$this->_padding[Pas_Pdf::BOTTOM];
				break;
			case Pas_Pdf::MIDDLE:
				$y=$posY+$this->_height/2+$line_height/2;
				break;
			default: //TOP
				$y=$posY+$line_height+$this->_padding[Pas_Pdf::TOP];
				break;
		}
		return $y;
	}
	...
	private function _getImagePosX($posX){
		...
		switch ($this->_align){
			case Pas_Pdf::RIGHT:
				$x=$posX+$this->_width-$this->_image['width']-$this->_padding[Pas_Pdf::RIGHT];
				break;
			case Pas_Pdf::CENTER:
				$x=$posX+$this->_width/2-$this->_image['width']/2;
				break;
			default: //LEFT
				$x=$posX+$this->_padding[Pas_Pdf::LEFT];
				break;
		}
		return $x;
	}
	...
	private function _getImagePosY($posY){
		...
		switch ($this->_vAlign){
			case Pas_Pdf::BOTTOM:
				$y=$posY+$this->_height-$this->_image['height']-$this->_padding[Pas_Pdf::BOTTOM];
				break;
			case Pas_Pdf::MIDDLE:
				$y=$posY+($this->_height-$this->_image['height'])/2;
				break;
			default: //TOP
				$y=$posY+$this->_padding[Pas_Pdf::TOP];
				break;
		}
		return $y;
	}
