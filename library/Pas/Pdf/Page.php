<?php
class Pas_Pdf_Page extends Zend_Pdf_Page {
	...
	public function getWidth($intContentArea=false){
		...
		if($intContentArea){
			$width-=$this->_margin[Pas_Pdf::LEFT];
			$width-=$this->_margin[Pas_Pdf::RIGHT];
		}
	 ...
	 * Get a Page margin
	 *
	 * @param Pas_Pdf::Position $position
	 * @return int margin
	 */
	 ...
	 * @param int $posY
	 */
	public function addTable(Pas_Pdf_Table $table,$posX,$posY,$inContentArea=true){
	...
	public function drawLine($x1,$y1,$x2,$y2,$inContentArea=true){
		//move origin
		if($inContentArea){
			$y1 = $this->getHeight()- $y1  - $this->getMargin(Pas_Pdf::TOP);
			$y2 = $this->getHeight()- $y2  - $this->getMargin(Pas_Pdf::TOP);
			$x1 = $x1  + $this->getMargin(Pas_Pdf::LEFT);
			$x2 = $x2  + $this->getMargin(Pas_Pdf::LEFT);
		}
...
//	public function drawImage($image,$x1,$y1,$width,$height,$inContentArea=){
//		if($inContentArea){
//			$y1 = $this->getHeight()- $y1  - $this->getMargin(Pas_Pdf::TOP)-$height;
//			$x1 = $x1  + $this->getMargin(Pas_Pdf::LEFT);
//
//			$y2=$y1+$height;
//			$x2=$x1+$width;
